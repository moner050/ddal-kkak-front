"""
데이터베이스 연결 및 설정 모듈 (MySQL 8.0.43)
SQLAlchemy를 사용한 MySQL 연결 관리
"""
...
from sqlalchemy import create_engine, text, Column, Integer, String, Numeric, Date, DateTime, Boolean, BigInteger
...
class DatabaseManager:
    ...
    def __init__(self, connection_string: Optional[str] = None):
        ...
        Args:
            connection_string: MySQL 연결 문자열 (없으면 환경변수에서 읽음)
        """
        ...
        self.engine = create_engine(
            ...
            max_overflow=10,
            pool_pre_ping=True,
            pool_recycle=3600,  # MySQL 연결 재사용 시간
            echo=False
        )
    ...
    def bulk_upsert_stocks(
        ...
        try:
            # 각 레코드에 data_date 추가 및 passed_profiles JSON 변환
            for record in stock_data:
                record['data_date'] = data_date

                # passed_profiles를 JSON 문자열로 변환 (리스트인 경우)
                if 'passed_profiles' in record and isinstance(record['passed_profiles'], list):
                    record['passed_profiles'] = json.dumps(record['passed_profiles'])
    ...
    def get_stocks_by_profile(
        ...
        """
        특정 프로필을 통과한 종목 조회 (MySQL JSON_CONTAINS 사용)
        ...
        try:
            ...
            if data_date:
                # MySQL JSON_CONTAINS 함수 사용
                query = session.query(UndervaluedStock).filter(
                    UndervaluedStock.data_date == data_date
                ).filter(
                    func.json_contains(
                        UndervaluedStock.passed_profiles,
                        f'"{profile_name}"'
                    ) == 1
                ).order_by(UndervaluedStock.total_score.desc()).limit(limit)
                ...
                for stock in query.all():
                    ...
                    # JSON 문자열을 리스트로 변환
                    if 'passed_profiles' in stock_dict and stock_dict['passed_profiles']:
                        if isinstance(stock_dict['passed_profiles'], str):
                            stock_dict['passed_profiles'] = json.loads(stock_dict['passed_profiles'])
...
if __name__ == "__main__":
    ...
    sample_data = [
        {
            ...
            'roe': 0.45,
            'total_score': 85.5,
            'passed_profiles': ['undervalued_quality', 'growth_quality']  # 리스트로 전달
        }
    ]
    ...
    stats = {
        ...
        'stage2_success': 95,
        'stage2_failed': 5,
        'errors': ['Error 1', 'Error 2'],  # 리스트로 전달
        'status': 'completed'
    }
